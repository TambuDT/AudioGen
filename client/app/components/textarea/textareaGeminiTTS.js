...
function TextsectionGemini({ voiceName }) {
  ...
  function buildRequestBody() {
    ...
    // Sostituisci parole tra {} con pronunce personalizzate
    const matches = text.match(/\{(.*?)\}/g);
    if (matches) {
      matches.forEach((match) => {
        const parolaOriginale = match.slice(1, -1);
        const pronunciaCustom = customPronunce.find(
          (item) => item.parola === parolaOriginale
        );
        if (pronunciaCustom?.sostituzione) {
  ...
  return (
    <div className="textsection-container">
      ...
      />
      <h4 className="text-section-subtitle">
        Inserisci il prompt per modificare lo stile
      </h4>
      <textarea
        value={promptStile}
        onChange={(e) => setpromptStile(e.target.value)}
        className="text-area"
        placeholder="Inserisci il prompt per modificare lo stile..."
      />
        ...
        {customPronunce.map((item, idx) => (
          <div key={idx} className="custom-pronuncia-row">
            ...
            <input
              ...
              onChange={(e) =>
                updatePronunce(idx, "sostituzione", e.target.value)
              }
...
export default TextsectionGemini;
